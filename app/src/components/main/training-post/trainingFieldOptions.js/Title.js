...
export const TitleComponent = ({ x, _id, token, setTraining }) => {
    ...
    const submitEditHandler = () => {
        if (toggleEdit.value.trim() != '' && toggleEdit.value.length > 3) {
            ...
            trainingService.editCntValue(toggleEdit.value, x._id, token)
                .then(res => {
                    if (!res.message) {
                        ...
                        setErrors({ message: 'You successfully edited title!', type: '' })
            ...
            setErrors({ message: 'Title must be at least 3 charaters!', type: '' })
        }
    }
        ...
        <>
            {
                toggleEdit.option
                    ...
                    :
                    < h2 > {x.author == _id && <i onClick={() => setToggleEdit({ value: x.value, option: true })} className="fa-solid fa-pen-to-square"></i>} {x?.value}</h2 >
            }
        </>
    )
}